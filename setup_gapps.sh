...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --update-repo)
      ...
      ;;
    *)
      echo "Usage: $(basename $0) { --update-repo | --lineage-verson }"
      exit 1
      ;;
  esac
done
...
if [ "$_UPDATE_REPO" ]; then
  apt install --no-install-recommends --no-install-suggests ca-certificates curl -y || exit $?
  curl https://repo.waydro.id | bash || exit $?
fi

apt install --no-install-recommends --no-install-suggests apparmor ca-certificates nftables waydroid -y || exit $?
